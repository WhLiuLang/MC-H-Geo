...
def main_goalkeeper():
    ...
    print("\n步骤 2: 动态计算差值/比值特征 ")
    X = df_multi.drop(columns=['label', 'filename'], errors='ignore')
    y = df_multi['label']

    new_features_list = []  # 1. 创建一个空列表来存放新特征
    epsilon = 1e-6

    # 2. 在循环中计算新特征，并添加到列表中
    for col_10cm in [c for c in X.columns if '_10cm' in c]:
        col_30cm = col_10cm.replace('_10cm', '_30cm')
        if col_30cm in X.columns:
            diff_col_name = col_10cm.replace('_10cm', '_diff')
            diff_series = X[col_30cm] - X[col_10cm]
            diff_series.name = diff_col_name
            new_features_list.append(diff_series)

            ratio_col_name = col_10cm.replace('_10cm', '_ratio')
            ratio_series = X[col_10cm] / (X[col_30cm] + epsilon)
            ratio_series.name = ratio_col_name
            new_features_list.append(ratio_series)

    # 3. 循环结束后，一次性合并所有特征
    if new_features_list:
        X = pd.concat([X] + new_features_list, axis=1)

    print(f"终极特征池构建完毕，总特征数: {X.shape[1]}")
    ...
    selector = RFECV(estimator=estimator, step=STEP, cv=cv, scoring='accuracy', n_jobs=-1, verbose=1)
    ...
    df_optimal = X[optimal_features].copy()
    df_optimal['label'] = df_multi['label']
    if 'filename' in df_multi.columns: df_optimal['filename'] = df_multi['filename']

    df_optimal.to_csv(OUTPUT_GOALKEEPER_CSV, index=False)

    print(f"\n--- 成功！---")
    print(f"新的“守门员”最优特征集已保存至: {OUTPUT_GOALKEEPER_CSV}")
